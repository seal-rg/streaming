"""
StreamQwen3_5 — Qwen3.5 (dense, 27B) with stream-specific modifications:
...
to enforce the same masking as full attention layers.
"""
...
# Import base components from transformers (reuse everything except what we override)
...
class StreamQwen3_5Attention(nn.Module):
    """Qwen3.5 full attention with stream modifications:
    - Channel-aware SDPA (role gating bias)
    - Attention output gating (Q proj outputs double width, second half is sigmoid gate)
    - Partial rotary embedding (only first rotary_dim of head_dim)
    """

    def __init__(self, config: StreamQwen3_5TextConfig, layer_idx: int):
        super().__init__()
        self.config = config
        ...
        self.gate_in_norm = Qwen3_5RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
    ...
    def forward(
        ...
        **kwargs,
        ...
        # Gating input (RMSNormed hidden states)
        gate_in = self.gate_in_norm(hidden_states)

        # Q projection -> split into query + output gate
        ...
        if self.role_gating_enabled and (channel_ids is not None):
            if channel_ids_kv is not None:
                ...
                raise ValueError("channel_ids_kv required in cache mode with role gating")
        ...
        # Apply attention output gating (sigmoid gate from Q projection)
        attn_output = attn_output * torch.sigmoid(gate)
        attn_output = self.o_proj(attn_output)
        return attn_output, attn_weights


class StreamQwen3_5DecoderLayer(GradientCheckpointingLayer):
    """Decoder layer that uses StreamQwen3_5Attention for full_attention layers
    and the original GatedDeltaNet for linear_attention layers."""
    ...
    def __init__(self, config: StreamQwen3_5TextConfig, layer_idx: int):
        ...
        if self.layer_type == "linear_attention":
            self.linear_attn = Qwen3_5GatedDeltaNet(config, layer_idx)  # type: ignore
            ...
            self.self_attn = StreamQwen3_5Attention(config, layer_idx)
        self.mlp = Qwen3_5MLP(config, config.intermediate_size)  # type: ignore
...
class StreamQwen3_5PreTrainedModel(PreTrainedModel):
    config: StreamQwen3_5TextConfig
    base_model_prefix = "model"
    supports_gradient_checkpointing = True
    _no_split_modules = ["StreamQwen3_5DecoderLayer"]
...
class StreamQwen3_5TextModel(StreamQwen3_5PreTrainedModel):
    """Qwen3.5 text model with channel embedding and stream position handling."""

    def __init__(self, config: StreamQwen3_5TextConfig):
        super().__init__(config)
        self.embed_tokens = nn.Embedding(config.vocab_size, config.hidden_size, getattr(config, "pad_token_id", None))
        self.layers = nn.ModuleList([StreamQwen3_5DecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)])
        self.norm = Qwen3_5RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.rotary_emb = Qwen3_5TextRotaryEmbedding(config=config)
        self.gradient_checkpointing = False

        # Stream: channel embedding
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],  # type: ignore
        ...
        if use_cache and past_key_values is None:
            past_key_values = Qwen3_5DynamicCache(config=self.config)  # type: ignore
        ...
        # Accept either:
        #   [B, S, 2] = [channel_id, row_position] (our custom format from collator)
        #   [B, S] or [3, B, S] (standard mRoPE)
        channel_ids = kwargs.pop("channel_ids", None)

        if position_ids is not None and position_ids.dim() == 3 and position_ids.size(-1) == 2:
            # Custom stream format: [B, S, 2] = [cid, y]
            channel_ids = position_ids[..., 0].long()
            local_y = position_ids[..., 1].long()
            # For mRoPE, broadcast local_y across all 3 dimensions (T=H=W=y)
            ...
            if position_ids is None:
                ...
                mrope_position_ids = position_ids[None, ...].expand(3, position_ids.shape[0], -1)
        ...
        # Handle 4D position_ids (strip text_position_ids dimension if present)
        ...
        if isinstance(attention_mask, dict):
            ...
            linear_attn_mask = attention_mask.get("linear_attention", attention_mask.get("sliding_attention", None))
        else:
        ...
        if self.channel_embedding_method != "none":
            ...
            hidden_states = hidden_states + channel_emb.to(hidden_states.device)  # type: ignore
    ...
    def _update_linear_attn_mask(self, attention_mask, cache_position):
        """Linear attention layers don't need the full causal mask."""
...
@auto_docstring
class StreamQwen3_5ForCausalLM(StreamQwen3_5PreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        super().__init__(config)
        self.model = StreamQwen3_5TextModel(config)
