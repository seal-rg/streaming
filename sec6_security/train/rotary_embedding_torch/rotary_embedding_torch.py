...
class RotaryEmbedding(Module):
    ...
    def rotary_freqs_from_coords_2d(self, coords_xy: torch.Tensor):
        """
        self: RotaryEmbedding(dim=head_dim//2, ...)  # 每轴占一半
        coords_xy: [B, N, 2]  (x,y) 可以是 float 或 int
        return: freqs [B, N, head_dim]  可广播到 [B, heads, N, head_dim]
        """
        x = coords_xy[..., 0]  # [B, N]
        y = coords_xy[..., 1]  # [B, N]

        # 注意：rope.forward(t) 支持 t 是任意形状，输出会是 [..., dim] (dim 已 repeat 成偶数维)
        freqs_x = self.forward(x)  # [B, N, head_dim//2]
        freqs_y = self.forward(y)  # [B, N, head_dim//2]
