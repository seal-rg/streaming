...
def is_correct_num(gold: str | None, pred: str | None) -> bool:
...
def load_examples(task: str, split: str | None, n: int, seed: int, in_jsonl: str | None) -> list[dict[str, Any]]:
    task = task.lower()
    if task == "squad":
        if in_jsonl:
...
def main():
    ...
    ap.add_argument("--task", type=str, required=True, choices=["gsm8k", "squad", "pubmedqa", "proofwriter", "logicnli"])
    ...
    ap.add_argument(
        ...
        type=str,
        default=None,
        help="optional local jsonl (for squad/proofwriter). For proofwriter can be wrapper lines containing {'data': {...}}",
    )
