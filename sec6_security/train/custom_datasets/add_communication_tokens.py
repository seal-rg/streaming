...
def main():
    ...
    parser.add_argument("--max-channels", type=int, default=10, help="Maximum number of channels to support (default: 10)")
    parser.add_argument("--no-save", action="store_true", help="Don't save the tokenizer (dry run)")
    parser.add_argument("--verify", action="store_true", default=True, help="Verify tokens after adding (default: True)")
    parser.add_argument("--test", action="store_true", help="Run encoding/decoding test")

    args = parser.parse_args()

    # Add tokens
    tokenizer, num_added = add_communication_tokens(
        tokenizer_path=args.tokenizer, output_path=args.output, max_channels=args.max_channels, save=not args.no_save
    )
