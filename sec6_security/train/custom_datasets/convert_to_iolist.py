...
def main():
    ...
    ap.add_argument("--input_key", type=str, default="input", help="Where to read the question/input text from (default: input).")
    ...
    ap.add_argument("--keep_meta", action="store_true", help="If set, keep gold/_quality in out_item['meta'].")

    ap.add_argument("--sample_id_key", type=str, default=None, help="Optional key to use as sample_id (besides existing sample_id).")
    ...
    with open(args.in_jsonl, encoding="utf-8") as fin, open(args.out_jsonl, "w", encoding="utf-8") as fout:
