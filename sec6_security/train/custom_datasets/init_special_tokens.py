...
def initialize_token_embeddings(model: PreTrainedModel, tokenizer: PreTrainedTokenizer, new_tokens: list[str]) -> dict:
    """
    ...
    for tok in new_tokens:
        ...
        if valid_ids:
            ...
            fallback_count += 1
            added_tokens_info[tok] = {"base_word": base_word, "method": "random_fallback"}
...
def verify_token_initialization(model: PreTrainedModel, tokenizer: PreTrainedTokenizer, sample_tokens: list[str] | None = None):
    """
    ...
    if sample_tokens is None:
        ...
        sample_tokens = ["<to:channel_0>", "</to>", "<chunk_0>", "</chunk_0>", "<to:all>"]
...
def main():
    ...
    model, tokenizer, added_info = add_and_init_communication_tokens(
        model, tokenizer, max_channels=args.max_channels, max_chunks=args.max_chunks
    )
    ...
    print("  2. Use this model for training with parallel chunk data")
    print("  3. The embeddings are initialized semantically from base words")
