...
class CacheBlock(transformers.cache_utils.Cache):
    """
    ...
    def __init__(self, config: transformers.PretrainedConfig, **kwargs):
        """
        Args:
            config: Model configuration (used for RoPE, not passed to parent)
            **kwargs: Additional arguments (not used, for compatibility)
        """
        # 保存 config 用于 RoPE 旋转
        self.config = config

        # 只传递 layer_classes 给父类，不传递 config
        # 这样 DynamicLayer 不会收到它不需要的参数
    ...
    def get_kv_with_offset(self, *, layer_idx: int, offset: int):
        ...
        assert len(self.layers) > layer_idx, "cache must be fully populated before it can accept None args"

        # 使用新的 layers API，处理 None 情况
        key_cache = self.layers[layer_idx].keys
        value_cache = self.layers[layer_idx].values

        # DynamicLayer 可能返回 None
        if key_cache is None or value_cache is None:
            raise ValueError(f"Layer {layer_idx} has not been populated yet")
    ...
    def append(
        ...
        self.first_available_positions_by_layer[layer_idx] = cache_positions_before_rotation[-1] + required_position_offset + 1
    ...
    def append_from(self, other: CacheBlock, keep_other: bool = True):
        ...
        for layer_idx in range(len(other.layers)):
            # 检查 other 的这一层是否有数据
            if other.layers[layer_idx].keys is None:
                continue
            ...
            # 获取旋转后的 key-value 对
            try:
                rotated_keys, rotated_values = other.get_kv_with_offset(layer_idx=layer_idx, offset=new_token_starting_position)
            except (ValueError, IndexError):
                # 如果 other 的这一层没有数据，跳过
                continue

            # cache_kwargs 设为 None，因为 other 已经旋转好了
            super().update(rotated_keys, rotated_values, layer_idx=layer_idx, cache_kwargs=None)
...
_ROTARY_CACHE: dict[Any, tuple[torch.Tensor, torch.Tensor]] | None = None
...
def test_cache_block(model_names=("meta-llama/Llama-3.2-3B", "Qwen/Qwen2.5-3B"), atol: float = 1e-4):
    for model_name in model_names:
        ...
        assert not torch.allclose(k[..., k0.shape[-2] :, :], k1, atol=atol)  # last 2 values are rotated to [12, 13] != [101, 102]
