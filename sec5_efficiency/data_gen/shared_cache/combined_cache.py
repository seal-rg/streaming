...
class CombinedCacheView(transformers.cache_utils.DynamicCache):
    """
    ...
    def __init__(
        ...
        write_to: Sequence[CacheBlock] | None = None,
        input_mask: torch.Tensor | None = None,
        position_ids: torch.Tensor | None = None,
        override_length: int | None = None,
        rotary_cache: dict | None = None,
    ):
        super().__init__()
    ...
    def get_seq_length(self, layer_idx: int | None = 0) -> int:
        ...
        worker_cache_lengths = [
            sum(cache_block.get_seq_length(layer_idx) for cache_block in worker_sequence) for worker_sequence in self.cache_structure
        ]
    ...
    def get_usable_length(self, new_seq_length: int, layer_idx: int | None = 0):
...
def combine_cache_from_structure(cache_structure: Sequence[Sequence[CacheBlock]], layer_idx: int) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    kv_parts: list[list[tuple[torch.Tensor, torch.Tensor] | None]] = [[None for _ in seq] for seq in cache_structure]
...
def collate_kv_with_left_padding(kv_parts: Sequence[Sequence[tuple[torch.Tensor, torch.Tensor]]]) -> tuple[torch.Tensor, torch.Tensor]:
    """
...
def test_combined_cache(model_names=("meta-llama/Llama-3.2-3B", "Qwen/Qwen2.5-3B"), atol: float = 1e-4):
    for model_name in model_names:
        ...
        combined_cache = CombinedCacheView(cache_structure, input_mask=torch.tensor([[True, True], [False, False], [True, False]]))
        ...
        w1_k_for_w23, w1_v_for_w23 = cache_w1.get_kv_with_offset(layer_idx=0, offset=5)
        assert torch.allclose(combined_keys_after[1:, ..., 7:11, :], w1_k_for_w23, atol=atol)  # note: 2 pad + 5 prompt = 7
        ...
        assert torch.allclose(combined_keys_after[1, ..., 11:13, :], w3_k_for_w2, atol=atol)  # note: 3 pad + 5 prompt + 4 w1 = 11
