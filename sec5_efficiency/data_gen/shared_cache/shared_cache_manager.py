...
class SharedCacheManager(nn.Module):
    def __init__(self, cache_structure: Sequence[Sequence[CacheBlock]], write_to: Sequence[CacheBlock] | None = None):
        """
    ...
    def get_input_kwargs(
        ...
        active_worker_indices: Sequence[int] | None = None,
    ) -> dict[str, torch.Tensor | CombinedCacheView]:
        """
        ...
        worker_new_seq_lengths = [
            sum(cache_block.get_seq_length() + extra_length_by_block[cache_block] for cache_block in worker_sequence)
        ...
        return dict(
            ...
            past_key_values=CombinedCacheView(
                ...
                write_to=write_to,
                input_mask=input_attention_mask,
                position_ids=position_ids,
                override_length=max_length_per_worker,
