...
def _beta_schedule(beta0: float, step: int, warmup: int, beta_max: float):
    if warmup <= 0:
        return min(beta0, beta_max)
    if step <= 0:
        return 0.0
    t = min(1.0, float(step) / float(warmup))
    return min(beta0 * t, beta_max)
...
class RoleGatingMLP(nn.Module):
    ...
    """
    def __init__(self, hidden_size: int, num_heads: int, num_roles: int,
    ...
    def forward(self, q_hidden: torch.Tensor, ctx: torch.Tensor | None = None):
        ...
        if self.granularity == "layer":
            ...
            return y.view(B, Q, self.num_heads, self.num_roles).permute(0, 2, 1, 3).contiguous()  # [B,H,Q,C]
...
class Qwen3Attention(nn.Module):
    def __init__(self, config, layer_idx: int):
        ...
        # ✅ stability knobs (simple constants)
        self.role_gating_tau = float(getattr(config, "role_gating_tau", 2.0))                    # ✅ tau=2
        self.role_gating_beta_max = float(getattr(config, "role_gating_beta_max", 0.8))          # ✅ beta=0.8 constant
        self.role_gating_log_eps = float(getattr(config, "role_gating_log_eps", 1e-4))           # ✅ eps=1e-4
        self.role_gating_log_clip_min = float(getattr(config, "role_gating_log_clip_min", -6.0))# ✅ clip=-6
        self.role_gating_uniform_mix = float(getattr(config, "role_gating_uniform_mix", 0.05))   # optional, e.g. 0.05

        # ✅ gate input RMSNorm (main fix)
    ...
    def forward(
        ...
        channel_ids_kv: torch.LongTensor | None = None,   # ✅ NEW
        **kwargs,
    ):
        ...
        if self.role_gating_enabled and (channel_ids is not None):
            ...
            if channel_ids_kv is not None:
                ...
                if past_key_value is None:
                    ...
                    channel_ids_k = None         # ✅ cache but no key-channel => disable gating prior safely
        ...
        # print(f"[DEBUG] Using attention implementation: {attention_interface.__name__}")
        ...
        attn_output, attn_weights = attention_interface(
            ...
            g=g,
            channel_ids_k=channel_ids_k,                 # ✅ now correct in cache mode
            beta=beta,
...
class Qwen3Model(Qwen3PreTrainedModel):
    def __init__(self, config: Qwen3MedusaConfig):
        ...
        self.num_channels = getattr(config, 'num_channels', 5)  # system, user, assistant0-5
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        # ------------------------------------------------------------
        # ✅ Robust parse of position_ids:
        #   - If [B,S,2] => (channel_id, local_y)
        #   - Else treat as standard [B,S] local_y; channel_ids defaults to 0
        # ------------------------------------------------------------
        ...
        channel_emb = self.channel_embedding(channel_ids)  # [B,S,D]
...
class Qwen3ForMedusa(Qwen3PreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        self.tokenizer = AutoTokenizer.from_pretrained(
            "${MODELS_ROOT}/Qwen--Qwen3-4B/snapshots/medusa"
            #"${MODELS_ROOT}/Qwen--Qwen3-4B-Base/snapshots/medusa"
    ...
    def forward(
        ...
        head_start: Optional[torch.Tensor] = None,   # [B,H] 或 [B] 或 [B,1]
        head_end: Optional[torch.Tensor] = None,     # [B,H] 或 [B] 或 [B,1]
        head_ok: Optional[torch.Tensor] = None,      # [B,H] 或 [B]

        boundaries: Optional[List[dict]] = None,     # 备选慢路径
    ...
    def _select_next_token(
        ...
        past_tokens: Optional[Iterable[int]] = None,  # assistant 已生成 token ids
    ) -> int:
        """
        Robust next-token sampling with STANDARD presence_penalty.

        presence_penalty:
        - if token appeared before (>=1), logits[token] -= presence_penalty
        - identical to OpenAI / HF "presence penalty" semantics
        """
        ...
        safe_logits = torch.nan_to_num(
            logits_1d, nan=-1e30, posinf=1e30, neginf=-1e30
        )
        ...
        # ============================================================
        # 🔹 STANDARD presence_penalty (OpenAI / transformers style)
        # ============================================================
    ...
    def medusa_generate_interleaved_v2_stream_user(
        ...
        # ===== NEW =====
        presence_penalty: float = 0.0,        # 标准 presence_penalty：出现过就减常数
    ) -> Dict[int, torch.Tensor]:
        """
        ...
        # 标准：只统计 assistant 采样出的 token（不含 system/user，不含 prefix）
    ...
    #     # list[str] / str(broadcast) / None
    #     assistant_prefix_texts=None,   # 放在 asst_prefix 后
    #     assistant_prefill_texts=None,  # 放在 asst_prefix 后
    ...
    #     ✅ 所有 assistant heads 的 y 都从同一个 y0 开始（各 head 的 y 轴对齐）
    #     ✅ head 间可见性：严格 y_key < y_query（不加 owner gate）
    #     """
    ...
    #         p_ids = tokenizer.encode(ptxt, add_special_tokens=False) if ptxt else []
    #         c_ids = tokenizer.encode(ctxt, add_special_tokens=False) if ctxt else []
    #         head_prefix_ids.append(p_ids)
    #         head_prefill_ids.append(c_ids)

    #     # ---------- Step0 ids ----------
    #     step0_ids: List[int] = []
    #     step0_ids += system_prefix + sys_ids + [im_end]   # system closed
    #     step0_ids += user_prefix                           # user prefix only
    ...
    #     # ---------- offsets ----------
    #     sys_len = len(system_prefix) + len(sys_ids) + 1  # includes im_end
    #     y0 = sys_len
    ...
    #     head_prefix_len = [len(head_prefix_ids[h]) for h in range(H)]
    #     head_prefill_len = [len(head_prefill_ids[h]) for h in range(H)]
    ...
    #     # ✅ assistants: 每个 head 都从 y0 开始（同 y0 起步）
    #     for h in range(H):
    #         owner = 2 + h
    ...
    #     # cross-channel y-rule STRICT: y_key < y_query
    #     y_to_keys: Dict[int, List[int]] = {}
    #     for j in range(S0):
    #         y_to_keys.setdefault(int(token_y0[j]), []).append(j)
    #     ys_sorted = sorted(y_to_keys.keys())
    ...
    #     # init: first token per head
    #     for h in range(H):
    #         bs, be = head_block_ranges[h]
    #         ctx_pos = (be - 1) if be > bs else (S0 - 1)
    #         hidden = out0.last_hidden_state[0, ctx_pos]
    #         logits = self.lm_head(hidden)
    ...
    #         generated = torch.cat([generated, torch.tensor([[first_tok]], device=device, dtype=torch.long)], dim=1)
    #         head_token_positions[h].append(generated.size(1) - 1)
    #         asst_gen_count[h] += 1

    #         pending_asst[h] = None if (stop_on_im_end and first_tok == im_end) else first_tok
    ...
    #         # 2) feed each head pending token
    #         for h in range(H):
    ...
    #             tok_a = int(pending_asst[h])
    #             active_kinds.append(f"asst_{h}")
    #             feed_tokens.append(tok_a)

    #             owner = 2 + h
    #             local_prefix_total = asst_prefix_len + head_prefix_len[h] + head_prefill_len[h]
    #             y_a = int(y0 + local_prefix_total + asst_gen_fed[h])  # ✅ 同 y0 起步
    #             y_a = max(0, min(y_a, int(self.max_position_embeddings) - 1))

    #             yq_list.append(y_a)
    #             cid_list.append(owner)
    ...
    #             # cached keys: STRICT y_key < y_query
    #             for j in range(cache_len):
    #                 if int(cache_token_y[j]) < yq:
    #                     rows[qi, j] = 0.0
    ...
    #         # advance pointers + sample next per head
    #         for i, kind in enumerate(active_kinds):
    #             if kind == "user":
    #                 user_ptr += 1
    #             else:
    #                 h = int(kind.split("_")[1])
    #                 asst_gen_fed[h] += 1
    ...
    #             continue
    #         pos0[bs:be, 0] = owner
    #         pos0[bs:be, 1] = y0 + torch.arange(Lh, device=device, dtype=torch.long)
    #         cache_owner0[bs:be] = owner

    #     token_y0 = pos0[:, 1].clone()  # [S0] long
    ...
    #     # cross-channel STRICT y_key < y_query (向量化一次性做)
    #     yq0 = token_y0.unsqueeze(1)  # [S0,1]
    #     yk0 = token_y0.unsqueeze(0)  # [1,S0]
    #     vis0 = (yk0 < yq0)           # [S0,S0]
    #     attn0 = torch.where(vis0, torch.zeros_like(attn0), attn0)
    ...
    #     # ============================================================
    #     # ✅ PREALLOC buffers (cache + generated)
    #     # ============================================================
    #     # worst-case cache length (safe upper bound)
    #     max_cache_len = S0 + len(user_stream) + H * max_new_tokens
    #     cache_y_buf = torch.empty((max_cache_len,), device=device, dtype=torch.long)
    #     cache_owner_buf = torch.empty((max_cache_len,), device=device, dtype=torch.long)

    #     cache_len = S0
    #     cache_y_buf[:S0] = token_y0
    #     cache_owner_buf[:S0] = cache_owner0

    #     # generated as python list (avoid torch.cat)
    #     generated_prefix = step0_ids[:]  # list[int]
    #     generated_tail: List[int] = []
    #     gen_len = len(generated_prefix)  # current total length
    ...
    #     # precompute masks up to Qmax=1+H
    #     Qmax = 1 + H
    #     diag_idx_qmax = torch.arange(Qmax, device=device)
    #     tri_prev = None
    ...
    #             owner = 2 + h
    #             local_prefix_total = asst_prefix_len + head_prefix_len[h] + head_prefill_len[h]
    #             y_a = int(y0 + local_prefix_total + asst_gen_fed[h])
    #             y_a = max(0, min(y_a, int(self.max_position_embeddings) - 1))
    ...
    #         inp = torch.as_tensor(feed_tokens, device=device, dtype=torch.long).unsqueeze(0)  # [1,Q]

    #         cid_t = torch.as_tensor(cid_list, device=device, dtype=torch.long)  # [Q]
    #         yq_t  = torch.as_tensor(yq_list,  device=device, dtype=torch.long)  # [Q]

    #         pos_q = torch.empty((1, Q, 2), device=device, dtype=torch.long)
    #         pos_q[0, :, 0] = cid_t
    #         pos_q[0, :, 1] = yq_t

    #         # ============================================================
    #         # ✅ FAST rows build (vectorized): STRICT y_key < y_query
    #         # ============================================================
    #         rows = torch.full((Q, key_len), _minf(torch.float32), device=device, dtype=torch.float32)

    #         cache_y = cache_y_buf[:cache_len]  # view [cache_len]
    #         vis_cache = cache_y.unsqueeze(0) < yq_t.unsqueeze(1)  # [Q, cache_len] bool
    #         rows[:, :cache_len] = torch.where(
    #             vis_cache,
    #             torch.zeros_like(rows[:, :cache_len]),
    #             rows[:, :cache_len],
    #         )

    #         # within-step self visible
    #         di = diag_idx_qmax[:Q]
    #         rows[di, cache_len + di] = 0.0

    #         # within-step allow previous visible
    #         if allow_same_step_visible and Q > 1:
    #             tri = tri_prev[:Q, :Q]  # [Q,Q]
    #             block = rows[:, cache_len:cache_len+Q]
    #             rows[:, cache_len:cache_len+Q] = torch.where(tri, torch.zeros_like(block), block)
    ...
    #         # ============================================================
    #         # ✅ append to cache buffers (no cat)
    #         # ============================================================
    #         if cache_len + Q > max_cache_len:
    #             # 理论上不会发生（因为 max_cache_len 是 worst-case），但防一下
    #             raise RuntimeError(f"cache buffer overflow: cache_len={cache_len}, Q={Q}, max_cache_len={max_cache_len}")

    #         cache_y_buf[cache_len:cache_len+Q] = yq_t
    #         cache_owner_buf[cache_len:cache_len+Q] = torch.as_tensor(owner_list, device=device, dtype=torch.long)
    #         cache_len += Q
    ...
    #                 assistant_histories[h].append(tok_next)
    #                 #print(f"assistant {h} generated token {tok_next}")
    ...
    #     # finalize generated tensor once
    ...
    def medusa_generate_interleaved_multihead_stream_user_same_y0(
        ...
        ===== 新增终止语义 =====
        - 只要“最后一个 head”(h=H-1) append 了 im_end：
            * 立刻终止其它 head（pending_asst 全置 None）
            * 并全局停止（break 主循环）：不再 feed user、不再生成
        ✅ 保留所有已 append 的 token（包括同一步已 append 的其它 head token）
        """
        ...
        # ---------- init: first token per head ----------
        # 若 last_head 在 init 就生成 im_end，则立刻 global_stop（但仍然保留已 append 的 token）
        for h in range(H):
            bs, be = head_block_ranges[h]
            ctx_pos = (be - 1) if be > bs else (S0 - 1)
            hidden = out0.last_hidden_state[0, ctx_pos]
            ...
            # append into generated_tail  (保留已append)
            generated_tail.append(first_tok)
            head_token_positions[h].append(gen_len)
            gen_len += 1
        ...
        # 如果 last head init 就触发了 stop：掐掉其它 head & 不再进入主循环
        if global_stop:
            for hh in range(H):
        ...
        for _ in range(max_steps):
            ...
            for i, kind in enumerate(active_kinds):
                if kind == "user":
                    ...
                    # append to generated list (保留已append)
                    generated_tail.append(tok_next)
                    head_token_positions[h].append(gen_len)
                    gen_len += 1
            ...
            # ===== 关键：last head 本 step 终止 => 掐掉其它 head + 全局停 =====
