...
PERSONA_TAG_RE = re.compile(
    r"^\s*<\s*(Solver|AltSolver|Auditor)\s*>\s*(.*)$",
    re.IGNORECASE
)
...
def parse_personas(teacher_output_cleaned: str) -> Tuple[Dict[str, str], Dict[str, List[str]]]:
    ...
    TRANSITION_SENTENCE = (
        "Based on the step-by-step reasoning and verification above, "
        "we can now summarize the key points and reach a conclusion."
    )
