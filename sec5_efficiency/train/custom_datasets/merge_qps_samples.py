...
def stable_hash(obj: Dict[str, Any], keys: Optional[List[str]] = None) -> str:
    """
...
def main():
    ...
    ap.add_argument("--take", nargs="+", required=True,
                    help='Per-file take counts, e.g. "/path/qps2.jsonl=1000" "/path/qps3.jsonl=800"')
    ...
    ap.add_argument("--dedup_keys", nargs="*", default=None,
                    help="Keys used for dedup hash, e.g. --dedup_keys id problem question")
    ap.add_argument("--strict", action="store_true",
                    help="If set, error when requested take > available, otherwise take all available.")
    args = ap.parse_args()
