...
class MedusaInference:
    def __init__(self, model_path: str, device: str = "auto", torch_dtype: str = "auto"):
        ...
        self.model: Optional[Qwen2ForMedusa] = None
    ...
    def _setup_special_token_ids(self):
        ...
        if self.wait_token_id is None or self.wait_token_id < 0:
            raise ValueError(
                f"Tokenizer missing {self.wait_token_str}. "
                f"Please load the tokenizer saved by your processor (save_tokenizer_to)."
            )

        print(
            f"[Tokens] im_start={self.im_start_id} im_end={self.im_end_id} "
            f"newline={self.newline_id} wait={self.wait_token_id}"
        )
